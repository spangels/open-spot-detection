...
def build_predictions_dataframe_on_folder(prediction_folder, video_folder,classes=CLASSES,
...
def build_predictions_dataframe(prediction_file, video_file,classes=CLASSES,
                                iou_threshold=0.2,min_persistance=4,conf_thres=0.4):
    df = {'left':[],'right':[]}
    counter =  CountSpots(video_path=video_file, txt_path=prediction_file,
            classes= CLASSES, iou_threshold=0.2, min_persistance=4,conf_thres=conf_thres)
    left, right = counter.count_preds()
    df['left'].append(left)
    df['right'].append(right)
...
def parse_opt():
    ...
    parser.add_argument('--video-file', type=str, default='', help='path of the folder cont')
    parser.add_argument('--prediction-file', type=str, default='', help='path of the folder cont')
...
def main(opt):
    video_file, prediction_file = opt.video_file, opt.prediction_file
    min_persistance, conf_thres = opt.min_persistance, opt.conf_thres
    iou_threshod = opt.iou_threshold
    save_file = opt.save_file
    df = build_predictions_dataframe(prediction_file, video_file,classes=CLASSES,
